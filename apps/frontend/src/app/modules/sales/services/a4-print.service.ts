...
export class A4PrintService {
  ...
  async printInvoice(orderDetails: any, orgDetails: any) {
    const html = this.generateInvoiceHTML(orderDetails, orgDetails);
    await this.printingService.printHTML(html);
  }

  generateInvoiceHTML(orderDetails: any, orgDetails: any): string {
    ...
    const invoiceSuffix = `${year}${month}${day}`;
        ...
        <body>
          ...
          <div class="billing-status-container">
            <div class="customer-info">
              ...
              <div class="customer-name">${orderDetails.customer_name || 'Walk-in Customer'}</div>
              ${orderDetails.customerEmail ? `<p class="customer-detail">${orderDetails.customerEmail}</p>` : ''}
              ${orderDetails.customerPhone ? `<p class="customer-detail">${orderDetails.customerPhone}</p>` : ''}
            </div>
          ...
          <table class="items-table">
            ...
            <tbody>
              ${items.map((item: any) => `
                <tr>
                  <td>${item.name}</td>
                  <td class="text-left">Ksh ${item.price.toLocaleString()}</td>
                  <td class="text-center">${item.selectedItems}</td>
                  <td class="text-right" style="font-weight:600;">Ksh ${(item.price * item.selectedItems).toLocaleString()}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
          ...
          <div class="totals-container">
            <div class="total-row">
              <span class="total-label">Service Subtotal:</span>
              <span class="total-value">Ksh ${orderDetails.total.toLocaleString()}</span>
            </div>
